        ...
        if (terrainOption == TerrainOption.deform)
        {
            ...
            if (Physics.Raycast(points[0] + new Vector3(0, 100, 0), Vector3.down, out raycastHit, Mathf.Infinity, ~(1 << LayerMask.NameToLayer("Road") | 1 << LayerMask.NameToLayer("Ignore Mouse Ray"))))
            {
                ...
                if (terrain != null)
                {
                    ...
                    modifiedHeights = terrainData.GetHeights(0, 0, terrainData.heightmapResolution, terrainData.heightmapResolution);
                    ...
                    for (int i = 0; i < points.Length; i++)
                    {
                        ...
                        for (float offset = 0; offset < 1; offset += 1 / zDivisions)
                        {
                            ...
                            for (float t = 0; t <= 1; t += 1f / divisions)
                            {
                                ...
                                int terrainPointX = (int)((localTerrainPoint.x / terrainData.size.x) * terrainData.heightmapResolution);
                                float terrainPointY = position.y / terrainData.size.y;
                                int terrainPointZ = (int)((localTerrainPoint.z / terrainData.size.z) * terrainData.heightmapResolution);

                                if (terrainPointX > terrainData.heightmapResolution || terrainPointZ > terrainData.heightmapResolution)
                                {
                                    continue;
                                }
